...
contract Collateralization {
    ...
    /// @notice Returns true if the deposit associated with the given ID exists and is funded, false
    /// otherwise.
    /// @param id ID of the associated deposit.
    function isFundedDeposit(uint128 id) public view returns (bool) {
        // TODO: shoud this (or another function) also check if the deposit has expired?
        return deposits[id].depositor != address(0);
    }
}
